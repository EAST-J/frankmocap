            ...
            if 'left_hand' in pred_output: # hand mocap
                for hand_type in pred_output:
                    if pred_output[hand_type] is not None:
                        ...
                        joints = pred_output[hand_type]['pred_joints_smpl']
                        wrist_root = joints[0:1].copy()
                        vertices -= wrist_root
                        pred_hand_root = pred_output[hand_type]['pred_hand_root']
                        pred_global_rot = pred_output[hand_type]['pred_global_rot']
                        pred_full_t = pred_output[hand_type]['pred_full_t']
                        K = pred_output[hand_type]['K']
                        # transform to the camera coordinate
                        vertices = vertices @ pred_global_rot.T + wrist_root - pred_hand_root + pred_full_t
...
def run_hand_mocap(args, bbox_detector, hand_mocap, visualizer):
    ...
    while True:
        ...
        vis = vis_utils.Visualizer(img_size=img_original_bgr.shape[:2])
        # pred_mesh_list = extract_mesh_from_output(pred_output_list)
        # res_img = vis.draw_mesh(np.array(img_original_bgr) / 255., pred_mesh_list[0]['vertices'], pred_mesh_list[0]['faces'].astype(np.int64), 
        #                         pred_mesh_list[0]['K'])[:, :, ::-1]*255
